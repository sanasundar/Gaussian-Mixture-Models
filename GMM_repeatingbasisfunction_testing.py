...
def to_test_model(features_to_split, landuse_categories_to_split, sites_to_split, models_for_testing, times):
    ...
    for train_index, test_index in sgkf.split(features_to_split,landuse_categories_to_split, groups = sites_to_split):
        ...
        time_test = np.take(times, test_index)
        time_test_dataframe = pd.DataFrame(time_test)
        ...
        finalDf_testing = pd.concat([x_pca_testing, time_test_dataframe], axis = 1)
...
for i in range(0, len(time_information)):
    ...
    x = time_rn[0]
    time_rn_integer = int(time_rn[0] + time_rn[1] + time_rn[2] + time_rn[3])
    time_integers_list.append(time_rn_integer)
time_integers_list = np.reshape(time_integers_list, (-1, 1))

rbf = RepeatingBasisFunction(n_periods = 24, input_range = (0, 2459))

time = rbf.fit_transform(time_integers_list)
print(time)

models_testing = pd.read_pickle(r"C:\Users\Saranya Sundar\OneDrive\Desktop\Data\Deciduous\deciduous_repeatingbasisfunction.pickle")

my_dictionary  = to_test_model(features, landuse_category, sites, models_testing, time)

with open(r'C:\Users\Saranya Sundar\OneDrive\Desktop\Data\Deciduous\deciduous_predictions_repeatingbasisfunction.pickle', 'wb') as handle:
