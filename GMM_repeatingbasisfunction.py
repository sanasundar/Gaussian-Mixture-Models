...
def to_separate_data(x_pca, landuse_dataframe, time_dataframe):
    ...
    finalDf_training = pd.concat([x_pca_training, time_dataframe], axis = 1)
...
def to_train_model(features_to_split, landuse_categories_to_split, sites_to_split, times):
    ...
    for train_index, test_index in sgkf.split(features_to_split, landuse_categories_to_split, groups = sites_to_split):
        ...
        time_train = np.take(times, train_index)
        ...
        time_dataframe_training = pd.DataFrame(time_train)
        ...
        finalDf_training_H, finalDf_training_L, finalDf_training_M = to_separate_data(principal_components_for_training, landuse_dataframe_training, time_dataframe_training)
...
my_dictionary = to_train_model(features_for_training, landuse_category_for_training, sites_for_training, time)

with open(r'C:\Users\Saranya Sundar\OneDrive\Desktop\Data\Evergreen\59290\evergreen_repeatingbasisfunction.pickle', 'wb') as handle:
