...
def to_test_model(features_to_split, landuse_categories_to_split, sites_to_split, models_for_testing, time_sin, time_cos):
    ...
    for train_index, test_index in sgkf.split(features_to_split,landuse_categories_to_split, groups = sites_to_split):
        ...
        time_sine_test = np.take(time_sin, test_index)
        time_cosine_test = np.take(time_cos, test_index)
        ...
        time_sin_dataframe = pd.DataFrame(time_sine_test)
        time_cos_dataframe = pd.DataFrame(time_cosine_test)
        finalDf_testing = pd.concat([x_pca_testing, time_sin_dataframe], axis = 1)
        finalDf_testing = pd.concat([finalDf_testing, time_cos_dataframe], axis = 1)
        print(finalDf_testing)

        gmm_H = models_for_testing[f'GMM_H {fold_no}']
        gmm_L = models_for_testing[f'GMM_L {fold_no}']
        gmm_M = models_for_testing[f'GMM_M {fold_no}']

        h_predictions = gmm_H.score_samples(finalDf_testing)
        l_predictions = gmm_L.score_samples(finalDf_testing)
        m_predictions = gmm_M.score_samples(finalDf_testing)
...
for i in range(0, len(time_information)):
    ...
    minute = int(time_rn[3] + time_rn[4])
...
models_to_test = pd.read_pickle(r"C:\Users\Saranya Sundar\OneDrive\Desktop\Data\Deciduous\deciduous_960_sine_cos_final.pickle")
print(models_to_test)

my_dictionary = to_test_model(features_for_training, landuse_category_for_training, sites_for_training, models_to_test, time_sin_list, time_cos_list)

with open(r'C:\Users\Saranya Sundar\OneDrive\Desktop\Data\Deciduous\deciduous_predictions_sine_cos.pickle', 'wb') as handle:
